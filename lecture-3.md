...
Perhaps we can also introduce a linguistic distinction here, for example between illocutionary and perlocutionary acts. This distinction was actually made by the philosopher Austin [@austinHowThingsWords2009]. It refers to the fact that speech acts are used to achieve certain goals. There are goals that are achieved simply because the speech act is accepted by the addressee and addressees, and there are also many that are achieved because something specific happens after the speech act. An example of an illocutionary act in which the goal is achieved by the speech act itself is an oath. Once the oath has been pronounced, the person who has sworn is obligated in a certain way. Only the utterance is necessary. But if I want to convince someone that something specific is the case&mdash;so that he or she then does something&mdash;then it is a perlocutionary act, i.e. an act in which something else is to be achieved by speaking.
